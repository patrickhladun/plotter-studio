...
install_axidraw_stack() {
  echo "[setup] Installing Axidraw CLI"
  local axidraw_zip_url="https://cdn.evilmadscientist.com/dl/ad/public/AxiDraw_API.zip"

  if pip install --upgrade "$axidraw_zip_url"; then
    echo "[setup] Axidraw API installed from official zip"
  elif pip install --upgrade pyaxidraw; then
    echo "[setup] pyaxidraw package installed"
  elif pip install --upgrade "git+https://github.com/evil-mad/axidraw.git#subdirectory=pyaxidraw"; then
    echo "[setup] pyaxidraw installed from GitHub"
  else
...
AXICLI_PATH="$APP_DIR/venv/bin/axicli"
if [ ! -x "$AXICLI_PATH" ]; then
  echo "[error] axicli not found in venv after install!" >&2
  exit 1
fi

touch "$ENV_FILE"
add_env_line "PLOTTERSTUDIO_AXICLI" "$AXICLI_PATH"
